...
contract("Cloudbric Token Sale deploy test", (accounts) => {
  ...
  beforeEach("setup contract for each test", async () => {
    ...
    sale = await CloudbricTokenSale.new(
      fund, token.address, { from: owner }
    );
    await token.setTokenSaleAmount(sale.address, 0);
  });

  it("Cloudbric Token owner should be same with owner", async () => {
    const tokenOwner = await token.owner();
    assert.equal(tokenOwner, owner);
  });

  it("Cloudbric Token Sale owner should be same with owner", async () => {
    const saleOwner = await sale.owner();
    assert.equal(saleOwner, owner);
  });
});

contract("Cloudbric Token Sale stage & round test", (accounts) => {
  const owner = accounts[0];
  const admin = accounts[1];
  const fund = accounts[2];
  const user = accounts[3];
...
contract("Cloudbric Token Sale Whielist Test", async (accounts) => {
  const owner = accounts[0];
  const admin = accounts[1];
  const fund = accounts[2];
...
contract("Cloudbric Token Sale Main Test", async (accounts) => {
  const owner = accounts[0];
  const admin = accounts[1];
  const fund = accounts[2];
  const user1 = accounts[3];
